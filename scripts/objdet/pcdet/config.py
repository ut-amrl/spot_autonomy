...
def merge_new_config(config, new_config, basepath):
    if '_BASE_CONFIG_' in new_config:
        with open(os.path.join(basepath, new_config['_BASE_CONFIG_']), 'r') as f:
            try:
    ...
    for key, val in new_config.items():
        ...
        if key not in config:
            config[key] = EasyDict()
        merge_new_config(config[key], val, basepath)

    return config


def cfg_from_yaml_file(cfg_file, config, basepath):
    with open(cfg_file, 'r') as f:
        ...
        merge_new_config(config=config, new_config=new_config, basepath=basepath)
