...
def depth(image: cv2.imread):
    ...
    model.load_state_dict(torch.load(f'repos/depthany2/metric_depth/checkpoints/depth_anything_v2_metric_{dataset}_{encoder}.pth', map_location=DEVICE))
    model.eval()
    model.to(DEVICE)
...
if __name__ == "__main__":
    ...
    ann_img, *_ = groundedsam(classes=["door"], image=cv2_img)
